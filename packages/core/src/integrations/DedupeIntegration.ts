    ...
    setup: (params: OrsIntegrationSetupParams) => {
      if (params.sdkLifeTimeEmitter) {
        params.sdkLifeTimeEmitter.on("reportError", (err: JsErrorInfoType) => {
          try {
            const throttleWait =
              typeof p?.throttleWait === "number" && p.throttleWait > 0
                ? p.throttleWait
                : null;
            const debounceWait =
              typeof p?.debounceWait === "number" && p.debounceWait > 0
                ? p.debounceWait
                : null;
            // 格式错误的err类型不作处理
            ...
            if (err && previousError && isSameError(err, previousError)) {
              ...
              if (
                throttleWait &&
                lastErrorTime &&
                performance.now() - lastErrorTime < throttleWait
              ) {
                markErrorAsDiscarded(err);
                return;
              }
              ...
              if (
                debounceWait &&
                lastErrorTime &&
                performance.now() - lastErrorTime < debounceWait
              ) {
...
function isSameError(
  currentError: JsErrorInfoType,
  previousError: JsErrorInfoType,
) {
