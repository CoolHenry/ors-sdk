...
export default class DataHandle {
    ...
    try {
      ...
      originSpan.forEach((span) => {
        ...
        for (const key in span) {
          const singleAttr = this.transformSpanAttr(
            spanType,
            key,
            span[key as keyof typeof span],
          );
  ...
  // 单个span的格式化
  private transformSpanAttr(
    type: keyof typeof alias = "resource",
    key: string,
    value: any,
  ) {
    return {
  ...
  dataConcat() {
    try {
      ...
      this.spanDataAttrs.forEach((attr: []) => {
        const curentspanAttr = attr.find(
          (item: any) => item.key === "rum.type",
        );
        // @ts-ignore
        const curentspanTypeAttr = curentspanAttr.value.stringValue;
        const startTimeAndEndTime = that.calculateTime(
          curentspanTypeAttr,
          attr,
        );
        const spanAttr = {
  ...
  calculateTime(type: string, dataAttrs: any[]) {
    try {
      ...
      const findStartTime = dataAttrs.find(
        (item) =>
          item.key === `${getType}.startTime` ||
          item.key === `${getType}.start.time`,
      );
      const findEndTime = dataAttrs.find(
        (item) =>
          item.key === `${getType}.endTime` ||
          item.key === `${getType}.end.time`,
      );
      let startTime: number | null = null;
      let endTime: number | null = null;
      findStartTime
        ? (startTime = findStartTime.value.stringValue)
        : (startTime = Date.now() * 1000000);
      findEndTime
        ? (endTime = findEndTime.value.stringValue)
