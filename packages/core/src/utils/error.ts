...
export const markErrorAsDiscarded = (err: JsErrorInfoType) => {
  ...
  (err as JsErrorInfoType & { _isDiscarded?: boolean })[DISCARDED_MARK_KEY] =
    true;
};

/** 判断当前错误信息是否被废弃 */
export const isErrorAsDiscarded = (
  err: JsErrorInfoType & { _isDiscarded?: boolean },
) => {
...
const getIgnoreErrorsList = (params: SessionParams) => {
  try {
    const initIgnoreErrorsList =
      params.ignoreErrors && Array.isArray(params.ignoreErrors)
        ? params.ignoreErrors
        : [];
    const errorBlackIgnoregList = windowOrs.samplingConfig.blJsErrMsg || [];
    const ignoreErrorsList = DEFAULT_IGNORE_ERRORS.concat(
      initIgnoreErrorsList,
    ).concat(errorBlackIgnoregList);
...
export const needSkipError = (message: string, params?: SessionParams) => {
  try {
    ...
    if (
      message &&
      Array.isArray(ignoreErrorsList) &&
      ignoreErrorsList?.length > 0
    ) {
      // 然后过滤非字符串项并判断
