...
type NavigationTimingPolyfillEntry = Omit<
  ...
  'initiatorType' | 'nextHopProtocol' | 'redirectCount' | 'transferSize' | 'encodedBodySize' | 'decodedBodySize' | 'type'
> & {
...
export const onFP = (cb: (arg0: { name: string; value: number | null }) => void): PerformanceObserver | undefined => {
  try {
...
/** 监听navigationTiming */
export const onNavigation = (cb: (arg0: { name: string; value: string }) => void) => {
  try {
