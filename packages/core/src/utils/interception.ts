...
export const isResourceBlacklisted = (url: string): boolean => {
  try {
    ...
    return blResUrl ? blResUrl?.length > 0 && blResUrl.some((item: string) => url.includes(item)) : false;
  } catch (error) {
...
export const urlIncludes = (entryName: string | undefined, params?: SessionParams): boolean => {
  try {
...
/** 是否应该上报过滤 */
export const needSkipUrlCollect = (url: string | undefined, params?: SessionParams): boolean => {
  try {
    const skipUrl = url && url.match(/^https?:\/\/[^/]+\/[^?]+/)?.[0];
    return skipUrl ? urlIncludes(skipUrl, params) || isResourceBlacklisted(skipUrl) : false;
  } catch (error) {
