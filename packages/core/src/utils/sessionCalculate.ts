...
const createSessionId = () => {
  try {
    ...
    sessionInfo.sessionStartTime = appInfo?.sessionStartTime || getCurrentTime();
    sessionStorage.setItem('orsSessionId', sessionInfo?.sessionId);
    sessionStorage.setItem('orsSessionStartTime', sessionInfo?.sessionStartTime.toString());
    ...
    windowOrs.orsDataInfo.sessionInfo.sessionStartTime = Math.floor(Number(sessionInfo?.sessionStartTime));
    ...
    if (windowOrs.samplingConfig.session) {
      sessionStorage.setItem('orsSessionSwitch', String(windowOrs.samplingConfig.session));
    }
    return sessionInfo;
  } catch (error) {
...
const generateOrGetSessionId = () => {
  try {
    ...
    const storedSessionStartTime = sessionStorage.getItem('orsSessionStartTime');
    const storedSessionIdTime = sessionStorage.getItem('orsSessionIdTime');
    if (sessionId) sessionInfo.sessionId = sessionId;
    if (storedSessionStartTime) sessionInfo.sessionStartTime = Number(storedSessionStartTime);
    // 检查存储的会话ID是否存在且未过期
    if (sessionId && storedSessionIdTime) {
      ...
      if (currentTime - storedTime < expirationTime) {
        ...
        // 如果会话采样率取过之后，复用上次的
        const orsSessionSwitch = sessionStorage.getItem('orsSessionSwitch');
        if (orsSessionSwitch) windowOrs.samplingConfig.session = JSON.parse(orsSessionSwitch);
