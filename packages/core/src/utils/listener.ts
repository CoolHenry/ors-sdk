...
const handleUnload = (sessionParams: SessionParams) => {
  try {
    ...
    const isNotSupportBeaconSendPbType = browser === 'Chrome' && Number(browserVersionMajor) < 81;
    if (reportData.length) {
      // 强制清空数据（即使上报失败）
      CollectStore.clear();
      // 尝试用 Beacon 上报
      const success = isNotSupportBeaconSendPbType ? false : reportClient.sendBeaconData(reportData);
