...
export default class ConsoleCollect extends Base {
    ...
    try {
      ...
      CONSOLE_LEVEL.forEach((level: ConsoleLevelType) => {
        ...
        fill(
          ...
          level,
          function (
            originalConsoleMethod: (...args: any[]) => any,
          ): (...args: any[]) => void {
            originalConsoleLevel[level] = originalConsoleMethod;

            return function (...args: any[]): void {
              try {
                const startTime = highTime(
                ...
                // sdk内部日志不进行上报
                const isSdkLogger = args.some(
                  (e) => isString(e) && e?.includes(ORS_SDK_LOGGER_KEY),
                );
  ...
  reportConsole(
    ...
    try {
      ...
      const isRateDrop =
        SamplingManager.decide({ rumType: "ors_console" }) === "drop";
      if (isRateDrop) {
        // 采集的数据存入store
