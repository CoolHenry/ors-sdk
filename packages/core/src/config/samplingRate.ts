...
import type { ISampleData, SamplingEventStatusType, CollectStoreType, SamplingManagerDecideParamsType, initObserveParams } from '@/types/init';
...
export class SamplingManager {
  ...
  public static init(rateConfig: ISampleData) {
    // 是否命中session采样率控制
    windowOrs.samplingConfig.session = samplingComputed('sessionRate', Number(rateConfig.sessionRate));
    const isHitSessionRate = windowOrs.samplingConfig.session;
    this.rules = {
      ors_view: isHitSessionRate && rateConfig.view && samplingComputed('viewRate', Number(rateConfig.viewRate)),
      ors_action: isHitSessionRate && rateConfig.action && samplingComputed('actionRate', Number(rateConfig.actionRate)),
      ors_resource: isHitSessionRate && rateConfig.resource && samplingComputed('actionRate', Number(rateConfig.resourceRate)),
      ors_error: rateConfig.jsError,
      ors_longtask: rateConfig.longtask,
      ors_console: isHitSessionRate && rateConfig.console && samplingComputed('consoleRate', Number(rateConfig.consoleRate)),
    };
    this.ready = true;
    Logger.log('[log][rate-rules]:', this.rules);
  }

  public static decide(item: CollectStoreType | SamplingManagerDecideParamsType): SamplingEventStatusType {
...
export const initConfig = (params: initObserveParams) => {
  //配置接口更新
  sdkLifeTimeUpdateEmitter.on('updateConfigRequest', (params: initObserveParams) => {
    getConfig(params);
  });
...
const getConfig = async (params: initObserveParams) => {
  // todo 白屏校验参数类型
  // 赋值白屏信息
  if (params?.plugins?.blankScreen) windowOrs.plugins.blankScreen = params.plugins.blankScreen;
  // 赋值采样率
