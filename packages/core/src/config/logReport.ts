  ...
  toReport(arg: {
    ...
    try {
      ...
      if (finalGzip && gzipUrl) {
        xhr.open(
          "POST",
          this.buildRequestUrl(gzipUrl, accessNo, webGroupId),
          true,
        );
      } else {
  ...
  request(
    ...
    try {
      return new Promise((resolve) => {
        ...
        xhr.open(
          "POST",
          this.buildRequestUrl(url, appInfo.accessNo, appInfo.webGroupId),
          true,
        );
...
export const logReport = async (type: string, value: any) => {
  try {
    const logSamplate = samplingComputed(
      "logRepRate" as SamplingType,
      Number(windowOrs?.samplingConfig?.logRepRate),
    );
